...
void Game2::setPhysicsWorld(PhysicsWorld* world) {
	m_world = world;
}
...
Scene* Game2::createScene()
{
	...
	auto layer = Game2::create(scene->getPhysicsWorld());
...
bool Game2::init(PhysicsWorld* world)
{
	if (!Layer::init())
	{
		return false;
	...
	//时间初始化
	time2 = 45;
	time_str = Label::createWithTTF("45", "fonts/Marker Felt.ttf", 36);
	time_str->setPosition(Vec2(visibleSize.width / 2, visibleSize.height - time_str->getContentSize().height));
	addChild(time_str, 3);
	//分数初始化
	Global::score2 = 0;
	//提示
	auto label = Label::create("在45秒内进行投篮\n(投篮要求：至少经过一次碰撞进球才有效！\n篮板、篮筐除外！)\n每投进一球加两分并续一秒\n结束时分数不低于四分可进入下一关！", "StartScene/font.ttf", 36);;
	...
	auto func = CallFunc::create(CC_CALLBACK_0(Game2::schedule_use, this));
...
//调用调度器
void Game2::schedule_use() {
	//时间调度器
	isbegin = true;
	this->schedule(schedule_selector(Game2::update_time), 1);
}

//变大函数
void Game2::label_scale(Label* s) {
	auto bigger = ScaleTo::create(0.5, 1.5);
	auto smaller = ScaleTo::create(0.5, 1);
	auto yellow1 = CallFunc::create(CC_CALLBACK_0(Game2::turn_yellow, this, s));
	auto white1 = CallFunc::create(CC_CALLBACK_0(Game2::turn_white, this, s));
	auto seq2 = Sequence::create(yellow1, bigger, white1, smaller, nullptr);
	s->runAction(seq2);
}

void Game2::turn_yellow(Label *y) {
	y->setColor(Color3B(235, 212, 61));
	y->enableGlow(Color4B(235, 212, 61, 0));
}
void Game2::turn_white(Label *w) {
	w->setColor(Color3B::WHITE);
	w->enableGlow(Color4B(185, 189, 172, 0));
}

//声音
void Game2::addvoice(string name)
{
...
//淡入淡出函数
void Game2::fade(float fadein, float fadeout, Label* a) {
...
//时间
void Game2::update_time(float t) {
	char d[10];
	if (time2 != 0) {
		time2--;
		itoa(time2, d, 10);
...
Game2* Game2::create(PhysicsWorld* world) {
	Game2* pRet = new(std::nothrow) Game2();
...
void Game2::addTouchListener() {
	auto touchListener = EventListenerTouchOneByOne::create();
	touchListener->onTouchBegan = CC_CALLBACK_2(Game2::onTouchBegan, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(Game2::onTouchMoved, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(Game2::onTouchEnded, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener, this);
}

void Game2::addMouseListener() {
	auto mouseListener = EventListenerMouse::create();
	mouseListener->onMouseMove = CC_CALLBACK_1(Game2::onMouseMove, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(mouseListener, this);
}

void Game2::addContactListener() {
	auto contactListener = EventListenerPhysicsContact::create();
	contactListener->onContactBegin = CC_CALLBACK_1(Game2::onConcactBegan, this);
	_eventDispatcher->addEventListenerWithFixedPriority(contactListener, 1);
}


void Game2::onMouseMove(Event* event) {
...
void Game2::onTouchMoved(Touch *touch, Event *unused_event) {
...
bool Game2::onConcactBegan(PhysicsContact& contact) {
	if (!istimeover && isbegin) {
		...
		if (nodeA && nodeB) {
			...
			if (nodeA->getPhysicsBody()->getTag() == 2 && nodeB->getPhysicsBody()->getTag() == 30) {
				if (nodeA->getPosition().y > nodeB->getPosition().y && duang > 0) {
					Global::score2 += 2;
					itoa(Global::score2, d, 10);
