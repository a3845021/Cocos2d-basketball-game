...
void Game3::setPhysicsWorld(PhysicsWorld* world) {
	m_world = world;
}
...
Scene* Game3::createScene()
{
	...
	auto layer = Game3::create(scene->getPhysicsWorld());
...
bool Game3::init(PhysicsWorld* world)
{
	if (!Layer::init())
	{
		return false;
	...
	//重力方向
	gravity = Sprite::create("gravity.png");
	gravity->setScale(0.4);
	gravity->setPosition(Vec2(gravity->getContentSize().width / 4, visibleSize.height - gravity->getContentSize().height / 4));
	addChild(gravity, 3);
	this->scheduleUpdate();

	//时间初始化
	time3 = 60;
	time_str = Label::createWithTTF("60", "fonts/Marker Felt.ttf", 36);
	time_str->setPosition(Vec2(visibleSize.width / 2, visibleSize.height - time_str->getContentSize().height));
	addChild(time_str, 3);
	//分数初始化
	Global::score3 = 0;
	//提示
	auto label = Label::create("此模式中每次发球前会随机产生重力的大小及方向！\n当前重力方向在左上角显示！\n每投进一球加三分并续一秒！\n结束时分数不低于六分即为闯关成功！", "StartScene/font.ttf", 36);;
	label->setPosition(Vec2(visibleSize.width - visibleSize.width / 3 - 120, visibleSize.height - label->getContentSize().height / 2 - 70));
	...
	auto func = CallFunc::create(CC_CALLBACK_0(Game3::schedule_use, this));
	...
	addChild(adds, 3);
	//随机生成重力
	m_world->setGravity(random_g());
	rotate(v);
	return true;
}
//调用调度器
void Game3::schedule_use() {
	//时间调度器
	isbegin = true;
	this->schedule(schedule_selector(Game3::update_time), 1);
}
//调整重力指示方向
void Game3::rotate(Vec2 v) {
	//初始化旋转动作
	rotate_action1 = RotateTo::create(1.0f, 180.0f);
	rotate_action2 = RotateTo::create(1.0f, 270.0f);
	rotate_action3 = RotateTo::create(1.0f, 90.0f);
	rotate_action4 = RotateTo::create(1.0f, 0.0f);
	if (v.x == 0 && v.y == -500)
		gravity->runAction(rotate_action1);
	else if (v.x == -200 && v.y == 0)
		gravity->runAction(rotate_action2);
	else if (v.x == 200 && v.y == 0)
		gravity->runAction(rotate_action3);
	else
		gravity->runAction(rotate_action4);
}
//音乐
void Game3::addvoice(string name)
{
...
void Game3::preloadmusic() {
	auto audio = SimpleAudioEngine::getInstance();
	audio->preloadBackgroundMusic("Sounds/Smack That.mp3");
	audio->preloadEffect("Sounds/db.mp3");
	audio->preloadEffect("Sounds/dd.mp3");
	audio->preloadEffect("Sounds/dk.mp3");
}
//变大函数
void Game3::label_scale(Label* s) {
	auto bigger = ScaleTo::create(0.5, 1.5);
	auto smaller = ScaleTo::create(0.5, 1);
	auto yellow1 = CallFunc::create(CC_CALLBACK_0(Game3::turn_yellow, this, s));
	auto white1 = CallFunc::create(CC_CALLBACK_0(Game3::turn_white, this, s));
...
//淡入淡出函数
void Game3::fade(float fadein, float fadeout, Label* a) {
...
//时间
void Game3::update_time(float t) {
	char d[10];
	if (time3 != 0) {
		time3--;
		itoa(time3, d, 10);
...
Game3* Game3::create(PhysicsWorld* world) {
	Game3* pRet = new(std::nothrow) Game3();
...
void Game3::addTouchListener() {
	auto touchListener = EventListenerTouchOneByOne::create();
	touchListener->onTouchBegan = CC_CALLBACK_2(Game3::onTouchBegan, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(Game3::onTouchMoved, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(Game3::onTouchEnded, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener, this);
}

void Game3::addMouseListener() {
	auto mouseListener = EventListenerMouse::create();
	mouseListener->onMouseMove = CC_CALLBACK_1(Game3::onMouseMove, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(mouseListener, this);
}

void Game3::addContactListener() {
	auto contactListener = EventListenerPhysicsContact::create();
	contactListener->onContactBegin = CC_CALLBACK_1(Game3::onConcactBegan, this);
	_eventDispatcher->addEventListenerWithFixedPriority(contactListener, 1);
}


void Game3::onMouseMove(Event* event) {
	//if (Isphysic == true && Basketball->getPhysicsBody()->getVelocity() == Vec2(0, 0) && IsthrowOver == true) {
	if (Isphysic == true
		&& Basketball->getPhysicsBody()->getVelocity().length() < 0.5//&& Basketball->getPosition().distance(Board->getPosition()) > 50
		&& IsthrowOver == true) {
...
bool Game3::onTouchBegan(Touch *touch, Event *unused_event) {
	if (Isphysic == false) {
	...
	if (Isphysic == true && Basketball->getPhysicsBody()->isGravityEnabled() == false) {
		Isthrow = true;
...
void Game3::onTouchMoved(Touch *touch, Event *unused_event) {
	MovingPos = touch->getLocation();
}

void Game3::onTouchEnded(Touch *touch, Event *unused_event) {
...
bool Game3::onConcactBegan(PhysicsContact& contact) {
